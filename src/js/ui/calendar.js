...
function renderCalendar(tasks) {
    ...
    for (let i = firstDayOfMonth - 1; i >= 0; i--) {
        calendarDays.appendChild(createDayElement(prevMonthDays - i, true, false, tasks, -1));
    }

    // Current month's days
    for (let i = 1; i <= daysInMonth; i++) {
        const isToday = i === new Date().getDate() && month === new Date().getMonth() && year === new Date().getFullYear();
        calendarDays.appendChild(createDayElement(i, false, isToday, tasks, 0));
    }
    ...
    for (let i = 1; i <= remainingCells; i++) {
        calendarDays.appendChild(createDayElement(i, true, false, tasks, 1));
    }
}
...
function createDayElement(dayNumber, isOtherMonth, isToday, tasks, monthOffset = 0) {
    ...
    // Show tasks on all days, including grayed out ones
